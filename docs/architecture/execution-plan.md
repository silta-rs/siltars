...
- `hybrid`: Rust owns HTTP, routing, JSON body parsing and HTTP serialization.
  ...
  Typed validation, request deadlines, and worker pooling remain future work.
...
3. Replace the single subprocess bridge with a supervised worker pool that
   supports deadlines, cancellation, backpressure, and restarts.
